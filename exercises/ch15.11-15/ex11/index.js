...
onmessage = function(message) {
    ...
    // ではなく、1つの整数として扱うことができる。この反復配列には、
    ...
    for(let row = 0, y = y0; row < height; row++, y += perPixel) {
        for(let column = 0, x = x0; column < width; column++, x += perPixel) {
            ...
            let n;             // これまでの反復回数。
            let r = x, i = y;  // z(0)をcに設定して開始する。
            for(n = 0; n < maxIterations; n++) {
                let rr = r*r, ii = i*i; // z(n)の2つの部分を自乗する。
                if (rr + ii > 4) {      // |z(n)|^2が4より大きければ、
...
class MandelbrotCanvas {
  constructor(canvas) {
      ...
      this.tiles = null;          // canvasの中の矩形。
      this.pendingRender = null;  // 現在は描画していない。
      this.wantsRerender = false; // 現在描画は要求されていない。
      this.resizeTimer = null;    // 頻繁なサイズ変更を防ぐ。
      this.colorTable = null;     // 生のデータをピクセル値に変換する。
  ...
  render() {
      ...
      // スキップして現在の状態を描画する。
      if (this.pendingRender) {        // すでに描画中であれば、
      ...
      this.pendingRender = Promise.all(promises).then(responses => {
          ...
          if (min === max) {                // すべてのピクセルが同じであれば、
              if (min === maxIterations) {  // すべてを黒色にする。
  ...
  handleResize(event) {
      ...
      this.resizeTimer = setTimeout(() => {
          ...
          this.setSize();           // canvasとtileのサイズを変更する。
          this.render();            // 新しいサイズで再描画する。
      }, 200);
  }
